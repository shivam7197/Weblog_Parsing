...
def _parse_weblog_file_forurls(filename):
...
def get_accessed_urls(filename):
	
	accessed_urls = _parse_weblog_file_forurls(filename)
	ips_dict = get_ips_with_count(filename)
	urls_dict = {}
	ips = _parse_weblog_file_forips(filename)
	#print ips
	for key in ips:
	#urls_dict[key] = accessed_urls
	    urls_dict[key] = urls_dict.setdefault(key,accessed_urls)
	
	
	print urls_dict.values()    
	#print urls_dict.items()
